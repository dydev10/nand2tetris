...
export const isBetweenPoints = (x: number, y: number, fromX: number, fromY: number, toX: number, toY: number): boolean => {
  return distanceToPoint(x, y, fromX, fromY) <= distanceToPoint(fromX, fromY, toX, toY)
    && distanceToPoint(x, y, toX, toY) <= distanceToPoint(fromX, fromY, toX, toY);
}
